...
: >OPCODE ( s -- opcode / 0 )
  ...
  DUP s" 1+"       STRING= IF DROP $05 EXIT THEN
  DUP s" 1-"       STRING= IF DROP $06 EXIT THEN
  DUP s" DUP"      STRING= IF DROP $07 EXIT THEN
  DUP s" DROP"     STRING= IF DROP $08 EXIT THEN
  DUP s" SWAP"     STRING= IF DROP $09 EXIT THEN
  DUP s" NIP"      STRING= IF DROP $0A EXIT THEN
  DUP s" OVER"     STRING= IF DROP $0B EXIT THEN
  DUP s" ROT"      STRING= IF DROP $0C EXIT THEN
  DUP s" -ROT"     STRING= IF DROP $0D EXIT THEN
  DUP s" TUCK"     STRING= IF DROP $0E EXIT THEN
  DUP s" INVERT"   STRING= IF DROP $0F EXIT THEN
  DUP s" AND"      STRING= IF DROP $10 EXIT THEN
  DUP s" OR"       STRING= IF DROP $11 EXIT THEN
  DUP s" XOR"      STRING= IF DROP $12 EXIT THEN
  ...
  DUP s" ="        STRING= IF DROP $17 EXIT THEN
  DUP s" <>"       STRING= IF DROP $18 EXIT THEN
  DUP s" JMP"      STRING= IF DROP $19 EXIT THEN
  DUP s" JZ"       STRING= IF DROP $1A EXIT THEN
  DUP s" JNZ"      STRING= IF DROP $1B EXIT THEN
  DUP s" AJMP"     STRING= IF DROP $1C EXIT THEN
  DUP s" CALL"     STRING= IF DROP $1D EXIT THEN
  DUP s" EXIT"     STRING= IF DROP $1E EXIT THEN
  DUP s" ."        STRING= IF DROP $1F EXIT THEN
  DUP s" NOP"      STRING= IF DROP $20 EXIT THEN
  DUP s" EMIT"     STRING= IF DROP $21 EXIT THEN
  DUP s" LIT"      STRING= IF DROP $22 EXIT THEN
  DUP s" %"        STRING= IF DROP $23 EXIT THEN
  DUP s" KEY"      STRING= IF DROP $24 EXIT THEN
  DUP s" SP"       STRING= IF DROP $25 EXIT THEN
  DUP s" SP!"      STRING= IF DROP $26 EXIT THEN
  DUP s" HALT"     STRING= IF DROP $27 EXIT THEN
  DUP s" LSHIFT"   STRING= IF DROP $28 EXIT THEN
  DUP s" RSHIFT"   STRING= IF DROP $29 EXIT THEN
  DUP s" !"        STRING= IF DROP $2A EXIT THEN
  DUP s" C!"       STRING= IF DROP $2B EXIT THEN
  DUP s" @"        STRING= IF DROP $2C EXIT THEN
  DUP s" C@"       STRING= IF DROP $2D EXIT THEN
  DUP s" DP"       STRING= IF DROP $2E EXIT THEN
  DUP s" DP!"      STRING= IF DROP $2F EXIT THEN
  DUP s" ,"        STRING= IF DROP $30 EXIT THEN
  ...
  DUP s" R@"       STRING= IF DROP $35 EXIT THEN
  DUP s" I"        STRING= IF DROP $35 EXIT THEN \ Alias to R@
  DUP s" EXEC"     STRING= IF DROP $36 EXIT THEN
  DUP s" DUMP"     STRING= IF DROP $37 EXIT THEN
  DUP s" ABORT"    STRING= IF DROP $38 EXIT THEN
  DUP s" [']"      STRING= IF DROP $39 EXIT THEN
  DUP s" '"        STRING= IF DROP $3A EXIT THEN
  DROP 0
;

...
: >OPCODE ( s -- opcode / 0 )
  ...
  DUP s" DEPTH"    STRING= IF DROP $2E EXIT THEN
  DUP s" >R"       STRING= IF DROP $2F EXIT THEN
  DUP s" R>"       STRING= IF DROP $30 EXIT THEN
  DUP s" R@"       STRING= IF DROP $31 EXIT THEN
  DUP s" I"        STRING= IF DROP $31 EXIT THEN \ Alias to R@
  DUP s" DUMP"     STRING= IF DROP $32 EXIT THEN
  DUP s" ABORT"    STRING= IF DROP $33 EXIT THEN
  DUP s" [']"      STRING= IF DROP $34 EXIT THEN

...
: >OPCODE ( s -- opcode / 0 )
  ...
  DUP s" I"        STRING= IF DROP $33 EXIT THEN \ Alias to R@
  DUP s" EXEC"     STRING= IF DROP $34 EXIT THEN
  DUP s" DUMP"     STRING= IF DROP $35 EXIT THEN
  DUP s" ABORT"    STRING= IF DROP $36 EXIT THEN
  DUP s" [']"      STRING= IF DROP $37 EXIT THEN
  DUP s" '"        STRING= IF DROP $38 EXIT THEN
  DROP 0
;

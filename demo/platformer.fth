...
: .VX 6 CELLS + ;
: .VY 7 CELLS + ;
: PLAYER_SPR PLAYER 8 CELLS + @ ;
: JUMPING PLAYER 9 CELLS + ;
...
: FACING
  PLAYER .VX @ 0 = IF
    LAST_FACING @
  ELSE
    PLAYER .VX @ SIGN DUP LAST_FACING !
  THEN ;
...
: TILE-W  PLAYER .WORLD_X @     PLAYER .WORLD_Y @     TILE C@ >TYPE ;
: TILE-E  PLAYER .WORLD_X @ 7 + PLAYER .WORLD_Y @     TILE C@ >TYPE ;
: TILE-SW PLAYER .WORLD_X @ 1+  PLAYER .WORLD_Y @ 8 + TILE C@ >TYPE ;
: TILE-SE PLAYER .WORLD_X @ 7 + PLAYER .WORLD_Y @ 8 + TILE C@ >TYPE ;
: TILE-NW PLAYER .WORLD_X @ 1+  PLAYER .WORLD_Y @     TILE C@ >TYPE ;
: TILE-NE PLAYER .WORLD_X @ 7 + PLAYER .WORLD_Y @     TILE C@ >TYPE ;
...
: AT-ENTRY? ( door -- bool )
  ( TODO rct )
  PLAYER .WORLD_X @ OVER .ENTRY.X PLAYER .WORLD_X @ 8 + BETWEEN?
  PLAYER .WORLD_Y @ ROT  .ENTRY.Y PLAYER .WORLD_Y @ 8 + BETWEEN?
  AND ;

: AT-EXIT? ( door -- bool )
  ( TODO rct )
  PLAYER .WORLD_X @ OVER .EXIT.X PLAYER .WORLD_X @ 8 + BETWEEN?
  PLAYER .WORLD_Y @ ROT  .EXIT.Y PLAYER .WORLD_Y @ 8 + BETWEEN?
  AND ;
...
: KEYBOARD-INPUT
  ...
  KEY_UP PRESSED?
  PLAYER .WORLD_X @ PLAYER .WORLD_Y @ TILE C@ 43 = AND IF
    ( TODO: chest )
    0 HALT
  THEN ;

: MOVE-X
  PLAYER .VX @ DT @ * PLAYER .SUB_X +=
  PLAYER .SUB_X @ 8 ARSHIFT ?DUP IF
    PLAYER .WORLD_X +=
    PLAYER .SUB_X $FF AND PLAYER .SUB_X !
  THEN ;

: MOVE-Y
  PLAYER .VY @ DT @ * PLAYER .SUB_Y +=
  PLAYER .SUB_Y @ 8 ARSHIFT ?DUP IF
    PLAYER .WORLD_Y +=
    PLAYER .SUB_Y $FF AND PLAYER .SUB_Y !
  THEN ;

: FINALIZE-X PLAYER .WORLD_X @ CAM_X @ - PLAYER_SPR X! ;
: FINALIZE-Y PLAYER .WORLD_Y @ PLAYER_SPR Y! ;

: FLIP   PLAYER_SPR PROP@ %00000010 OR  PLAYER_SPR PROP! ;
: UNFLIP PLAYER_SPR PROP@ %11111101 AND PLAYER_SPR PROP! ;
...
: COLLISION-X
  PLAYER .VX @ 0 < IF
    TILE-W SOLID = IF
      PLAYER .WORLD_X @ $FFF8 AND $08 + PLAYER .WORLD_X !
      0 PLAYER .SUB_X !
      0 PLAYER .VX !
    THEN
  THEN
  PLAYER .VX @ 0 > IF
    TILE-E SOLID = IF
      PLAYER .WORLD_X @ $FFF8 AND PLAYER .WORLD_X !
      0 PLAYER .SUB_X !
      0 PLAYER .VX !
    THEN
  THEN ;
...
: COLLISION-Y
  PLAYER .VY @ 0 > IF
    TILE-SW SOLID = 
    TILE-SE SOLID = OR IF
      PLAYER .WORLD_Y @ $FFF8 AND PLAYER .WORLD_Y !
      0 PLAYER .VY !
      0 PLAYER .SUB_Y !
    THEN
  THEN
  PLAYER .VY @ 0 < IF
    TILE-NW SOLID = 
    TILE-NE SOLID = OR IF
      PLAYER .WORLD_Y @ $FFF8 AND $09 + PLAYER .WORLD_Y !
      0 PLAYER .VY !
      0 PLAYER .SUB_Y !
    THEN
  THEN ;
...
: SCROLL
  PLAYER .WORLD_X @ SCROLL_START_X_RIGHT > IF
    PLAYER .WORLD_X @ SCROLL_START_X_RIGHT -
    0 MAX 256 MIN
    CAM_X !
  THEN
...
BEGIN
  ...
  DT @ 16 > IF   \ 60 FPS
    ...
    \ " X=" PRINT PLAYER_SPR X@ . "  Y=" PRINT PLAYER Y@ .
    \ "  WX=" PRINT PLAYER .WORLD_X @ . "  WY=" PRINT PLAYER .WORLD_Y @ .
   ...
   \ PLAYER_SPR Y@ 8 + 224 >= IF 0 HALT THEN
    TICKS TIMER !

  THEN

  ANIM_TIMER @ TICKS - ABS 150 > IF
    PLAYER_SPR ANIMATION @ ANIMATE
    TICKS ANIM_TIMER !
  THEN
